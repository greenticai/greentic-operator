...
impl DemoRunnerHost {
    ...
    pub fn supports_op(&self, domain: Domain, provider_type: &str, op_id: &str) -> bool {
        let result = self.catalog
            ...
            .map(|pack| {
                let has_flow = pack.entry_flows.iter().any(|flow| flow == op_id);
                let has_pack_op = pack_supports_provider_op(&pack.path, op_id).unwrap_or(false);
                eprintln!(
                    "[DEBUG supports_op] provider={} op={} has_flow={} has_pack_op={} entry_flows={:?}",
                    provider_type, op_id, has_flow, has_pack_op, pack.entry_flows
                );
                has_flow || has_pack_op
            })
            .unwrap_or_else(|| {
                eprintln!(
                    "[DEBUG supports_op] provider={} NOT in catalog. keys={:?}",
                    provider_type,
                    self.catalog.keys().map(|(d,p)| format!("{:?}/{}", d, p)).collect::<Vec<_>>()
                );
                false
            });
        result
    ...
    pub fn invoke_provider_op(
        ...
        eprintln!("[DEBUG invoke_provider_op] provider={} op={} payload_len={}", provider_type, op_id, payload_bytes.len());
