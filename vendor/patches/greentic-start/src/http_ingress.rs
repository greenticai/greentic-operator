...
impl HttpIngressServer {
    pub fn start(config: HttpIngressConfig) -> Result<Self> {
        ...
        let handle = thread::Builder::new()
            ...
            .spawn(move || -> Result<()> {
                eprintln!("[DEBUG thread] ingress thread started, creating runtime...");
                let runtime = Runtime::new().context("failed to create ingress runtime")?;
                eprintln!("[DEBUG thread] runtime created, binding to {}...", addr);
                runtime.block_on(async move {
                    let listener = TcpListener::bind(addr)
                        .await
                        .context("failed to bind ingress listener")?;
                    eprintln!("[DEBUG thread] bound to {}, accepting connections...", addr);
                    operator_log::info(
                        module_path!(),
                    ...
                    loop {
                        tokio::select! {
                            ...
                            accept = listener.accept() => match accept {
                                Ok((stream, peer)) => {
                                    eprintln!("[DEBUG accept] new connection from {}", peer);
                                    let connection_state = state.clone();
                                    tokio::spawn(async move {
                                        eprintln!("[DEBUG accept] serving connection from {}", peer);
                                        ...
                                        match http
                                            ...
                                            .await
                                        {
                                            Ok(()) => eprintln!("[DEBUG accept] connection from {} completed OK", peer),
                                            Err(err) => {
                                                eprintln!("[DEBUG accept] connection from {} ERROR: {}", peer, err);
                                                operator_log::error(
                                                    module_path!(),
                                                    format!(
...
async fn handle_request(
    ...
    eprintln!("[DEBUG ingress] {} {}", method, path);
    let response = match handle_request_inner(req, state).await {
        Ok(response) => {
            eprintln!("[DEBUG ingress] {} {} -> OK status={}", method, path, response.status());
            with_cors(response)
        }
        Err(response) => {
            eprintln!("[DEBUG ingress] {} {} -> ERR status={}", method, path, response.status());
            with_cors(response)
        }
    };
    Ok(response)
...
async fn handle_request_inner(
    ...
    let path = req.uri().path().to_string();
    eprintln!("[DEBUG inner] method={} path={}", req.method(), path);
    ...
    let method = req.method().clone();
    eprintln!("[DEBUG inner] parsing route segments...");
    let parsed = match parse_route_segments(req.uri().path()) {
        Some(value) => {
            eprintln!("[DEBUG inner] parsed: domain={:?} provider={} tenant={} team={}", value.domain, value.provider, value.tenant, value.team);
            value
        }
        None => {
            eprintln!("[DEBUG inner] route parse FAILED");
            return Err(error_response(
    ...
    if !state.domains.contains(&domain) {
        eprintln!("[DEBUG inner] domain {:?} disabled", domain);
        return Err(error_response(StatusCode::NOT_FOUND, "domain disabled"));
    }
    let has_op = state
        .runner_host
        .supports_op(domain, &parsed.provider, "ingest_http");
    eprintln!("[DEBUG inner] supports_op(provider={}, op=ingest_http) = {}", parsed.provider, has_op);
    if !has_op {
        return Err(error_response(
    ...
    eprintln!("[DEBUG inner] calling dispatch_http_ingress provider={} tenant={}", parsed.provider, parsed.tenant);
    let result = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        Ok(Ok(result)) => {
            eprintln!("[DEBUG inner] dispatch OK: events={} envelopes={}", result.events.len(), result.messaging_envelopes.len());
            result
        }
        Ok(Err(err)) => {
            eprintln!("[DEBUG inner] dispatch ERROR: {}", err);
            return Err(error_response(StatusCode::BAD_GATEWAY, err.to_string()));
        }
        Err(panic_info) => {
            ...
            eprintln!("[DEBUG inner] dispatch PANIC: {}", msg);
            return Err(error_response(StatusCode::INTERNAL_SERVER_ERROR, format!("panic: {msg}")));
        }
    };
