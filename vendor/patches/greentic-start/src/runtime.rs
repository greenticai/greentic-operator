...
pub fn demo_up_services(
    ...
    // Start embedded HTTP ingress in non-GSM (embedded) mode.
    let ingress_server = if !run_gsm_services {
        let secrets_handle = crate::secrets_gate::resolve_secrets_manager(
            config_dir,
            tenant,
            Some(team),
        )?;
        let runner_host = std::sync::Arc::new(crate::runner_host::DemoRunnerHost::new(
        ...
        if discovery.domains.messaging {
            enabled_domains.push(crate::domains::Domain::Messaging);
        }
        if discovery.domains.events {
            enabled_domains.push(crate::domains::Domain::Events);
        }
        ...
        .parse()
        .with_context(|| "invalid gateway listen address")?;
        let ingress_config = HttpIngressConfig {
            bind_addr,
            domains: enabled_domains,
            runner_host,
        };
        let server = HttpIngressServer::start(ingress_config)?;
        println!("HTTP ingress ready at http://{}", bind_addr);
        Some(server)
    } else {
        None
    };
