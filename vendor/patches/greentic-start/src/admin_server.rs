...
impl AdminServer {
    pub fn start(config: AdminServerConfig) -> Result<Self> {
        ...
        let handle = thread::Builder::new()
            ...
            .spawn(move || -> Result<()> {
                let runtime = Runtime::new().context("failed to create admin runtime")?;
...
fn build_tls_config(
    ...
    let client_verifier =
        rustls::server::WebPkiClientVerifier::builder_with_provider(Arc::new(root_store), provider.clone())
            .build()
...
async fn route_admin_request(
    ...
    match (method.clone(), path) {
        ...
        (Method::POST, "/admin/v1/deploy") => {
            ...
            handle_deploy(state, body)
        }

        (Method::POST, "/admin/v1/remove") => {
            let body = read_json_body(req).await?;
            handle_remove(state, body)
        }

        (Method::POST, "/admin/v1/setup") => {
            let body = read_json_body(req).await?;
            handle_setup(state, body)
        }
        ...
        _ => {
            ...
            Err(json_response(
                ...
                json!(AdminResponse::<()>::err(format!("unknown endpoint: {path}"))),
            ))
        }
    }
}
...
fn handle_status(state: &AdminState) -> Result<Response<Full<Bytes>>, Response<Full<Bytes>>> {
    ...
    let (pack_count, provider_count) = match discovery::discover(bundle) {
        Ok(result) => (
    ...
    Ok(json_response(StatusCode::OK, json!(AdminResponse::ok(resp))))
}
...
fn handle_deploy(
    ...
    let req: BundleDeployRequest = serde_json::from_value(body)
        .map_err(|err| json_response(StatusCode::BAD_REQUEST, json!(AdminResponse::<()>::err(err.to_string()))))?;

    let config = SetupConfig {
        tenant: req.tenants.first().map(|t| t.tenant.clone()).unwrap_or_else(|| "demo".into()),
    ...
    let plan = engine
        ...
        .map_err(|err| json_response(StatusCode::INTERNAL_SERVER_ERROR, json!(AdminResponse::<()>::err(err.to_string()))))?;

    if req.dry_run {
        let steps: Vec<String> = plan.steps.iter().map(|s| s.description.clone()).collect();
        return Ok(json_response(StatusCode::OK, json!(AdminResponse::ok(json!({
            "dry_run": true,
            "steps": steps,
        })))));
    }
    ...
    Ok(json_response(StatusCode::OK, json!(AdminResponse::ok(json!({
        "deployed": true,
...
fn handle_remove(
    ...
    let req: BundleRemoveRequest = serde_json::from_value(body)
        .map_err(|err| json_response(StatusCode::BAD_REQUEST, json!(AdminResponse::<()>::err(err.to_string()))))?;
...
fn handle_setup(
    ...
    let tenant = body.get("tenant").and_then(|v| v.as_str()).unwrap_or("demo");
...
async fn handle_qa_submit(
    ...
    let (provider_id, tenant, team, all_answers) = {
        ...
        let session = sessions.get_mut(session_id).ok_or_else(|| {
            json_response(
                StatusCode::NOT_FOUND,
                json!(AdminResponse::<()>::err(format!("session {session_id} not found"))),
            )
        })?;
        if session.is_expired() {
...
async fn read_json_body(
    req: Request<Incoming>,
) -> Result<JsonValue, Response<Full<Bytes>>> {
    let bytes = req
        .into_body()
        .collect()
        .await
        ...
        .map_err(|err| {
            json_response(
                StatusCode::BAD_REQUEST,
                json!(AdminResponse::<()>::err(format!("failed to read body: {err}"))),
            )
        })?;
    ...
    Response::builder()
        ...
        .unwrap_or_else(|err| {
            Response::builder()
                ...
                .body(Full::from(Bytes::from(format!("response build error: {err}"))))
                .unwrap()
        })
}
