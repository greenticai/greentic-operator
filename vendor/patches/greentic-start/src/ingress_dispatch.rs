...
pub fn dispatch_http_ingress(
    ...
    eprintln!("[DEBUG dispatch] provider={} tenant={} domain={:?}", request.provider, request.tenant, domain);
    ...
    eprintln!("[DEBUG dispatch] invoking provider op ingest_http payload_len={}", payload_json.len());
    ...
    eprintln!("[DEBUG dispatch] outcome: success={} error={:?}", outcome.success, outcome.error);
    if !outcome.success {
        let message = outcome
            .error
            .or(outcome.raw)
            .unwrap_or_else(|| "provider ingest_http failed".to_string());
        eprintln!("[DEBUG dispatch] provider FAILED: {}", message);
        anyhow::bail!("{message}");
    }
