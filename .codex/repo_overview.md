...
- **Path:** `src/bundle_access.rs`
  - **Role:** PR-02 bundle runtime access selection layer.
  - **Key functionality:** Selects directory vs SquashFS runtime access, prefers OS-mounted SquashFS when supported, falls back to `unsquashfs` userspace extraction when mount is unavailable, tracks active runtime root plus fallback diagnostics, performs best-effort cleanup on drop, and now exposes the shared `operator_bundle_access_config(...)`, `with_operator_bundle_read_root(...)`, and `operator_bundle_cbor_only(...)` helpers used for the standard `state/runtime/bundle_fs` staging policy, simple staged read-root access, and bundle-mode detection; image bundles now always stage through that read-root helper instead of falling back to the raw `.sqsh` path, and the demo CLI setup flow now consumes that shared helper too instead of opening bundle access directly.
  - **Key dependencies / integration points:** Used by `demo::runner_host` to stage read-only bundle access for discovery/runtime reads while leaving writable operator state on the original bundle root; onboard provider listing, onboard QA-spec generation, and component-QA apply flows now use staged-read-root discovery instead of assuming direct filesystem access to the original bundle path, demo ingress/timer event routing plus messaging app-pack lookup split staged read roots from writable run-artifact roots, the universal messaging ingress/egress helpers and universal subscription demo bootstrap now stage bundle access before discovery so they follow the same split-root model, both `demo_doctor` and `doctor` now stage bundle access before scanning provider packs so they no longer assume an unpacked directory bundle, `demo::setup` now exposes split-root tenant discovery for the same reason, and the demo CLI pack/setup/run/send/capability helpers now resolve CBOR validation, provider inventories, discovery, and app-pack roots through the staged read root instead of reading the raw bundle path directly.
- **Path:** `src/discovery.rs`
  - **Role:** Provider/domain discovery for runtime and demo tooling.
  - **Key functionality:** Scans provider pack roots, reads pack ids from manifests, persists detected domains/providers, and now exposes `discover_bundle_with_options(...)` plus shared runtime helpers like `bundle_cbor_only(...)`, `discover_runtime_bundle(...)`, `discover_bundle_auto(...)`, `discover_bundle_cbor_only(...)`, and `discover_validated_bundle_cbor_only(...)`; the plain `discover(...)` entrypoint also routes through staged bundle access, so bundle-image callers no longer need to pre-stage discovery manually.
  - **Key dependencies / integration points:** Used by demo CLI startup/send/capability flows, universal ingress/egress, universal subscription bootstrap, component-QA flows, onboard QA generation, `messaging_universal::provider`, `demo::runner_host`, and other runtime helpers; PR-02 work is progressively moving callers from raw-root discovery to the staged bundle-aware entrypoints and shared CBOR-mode detection.
- **Path:** `src/domains/mod.rs`
  - **Role:** Provider-pack discovery, validation, and planning helpers.
  - **Key functionality:** Defines domain configs, discovers provider packs, validates `manifest.cbor` presence, resolves validator packs, and now exposes `ensure_bundle_cbor_packs(...)`, `discover_bundle_provider_packs(...)`, `discover_bundle_provider_packs_auto(...)`, and `discover_provider_packs_with_options(...)` so validation and provider-pack enumeration can run through staged bundle access and shared CBOR-mode decisions for image-backed bundles.
  - **Key dependencies / integration points:** Used across demo CLI setup/send/capability/start flows, messaging provider helpers, onboarding pack discovery, `demo::runner_host`, and runtime planning; PR-02 work is progressively moving bundle-image-sensitive validation and pack enumeration from raw-root callers to the staged bundle-aware helpers.
- **Path:** `src/runtime_core.rs`
  - **Role:** PR-01 runtime-core foundation layer.
  - **Key functionality:** Defines operator-facing seam traits (`SessionProvider`, `StateProvider`, `TelemetryProvider`, `ObserverHookSink`, `AdminAuthorizationHook`, `BundleSource`, `BundleResolver`, `BundleFs`), discovers capabilities/hooks/subscriptions from pack metadata, builds a runtime wiring plan over the discovered offers, and exposes helpers for runtime event publication plus wiring-plan role/hook/subscription lookups.
  - **Key dependencies / integration points:** Builds on `capabilities` and `offers`; currently instantiated by `demo::runner_host` for startup wiring, capability resolution, pre/post runtime event emission, validation that configured universal-subscription providers actually advertise subscription offers, post-ingress hook resolution without rediscovering offer packs from disk, live runtime event/log payloads that include selected provider-role snapshots plus seam-backed state/session diagnostics, onboard API authorization through the admin-auth seam, explicit `runtime.dependency_unavailable`, `runtime.dependency_degraded_execution`, and `runtime.load_shed` audit events on the direct-provider path, ingress phase events (`ingress.received` / `ingress.routed`) emitted from the shared HTTP ingress dispatch path, post-ingress hook phase events (`hook.chain_loaded`, `hook.invoked`, `hook.directive_applied`, `hook.directive_error`, `hook.invocation_failed`) emitted from the shared hook runner, and bounded telemetry/observer event delivery with operator-side timeout/failure accounting plus a `drop_when_busy` backpressure policy surfaced in runtime status.
- **Path:** `src/demo/runner_host.rs`
  - **Role:** PR-07 provider-health and safe-mode orchestration layer.
  - **Key functionality:** Builds the live runtime host over seam-backed session/state/admin/bundle defaults, tracks generic provider-class health for `session`, `state`, `telemetry`, `observer`, and bundle access seams, computes `safe_mode` plus `degraded_level`, and uses that model as the source of truth for `/readyz`, `/status`, direct-execution refusal, and runtime-mode reporting.
  - **Key dependencies / integration points:** Wraps `runtime_core`, `bundle_lifecycle`, `bundle_access`, and runner execution; now emits provider outage/recovery, safe-mode enter/leave, degraded-level change, and load-shedding audit events, keeps `dependencies` as a compatibility projection of the richer `provider_health` snapshot, treats optional unconfigured seams as `unknown` so they do not trigger safe mode by themselves, exposes a shared request-policy enforcement path used by control-plane deployment/config/cache mutations, HTTP ingress, post-ingress hooks, background messaging egress, and default event-router delivery to refuse unsafe execution with explicit provider-class reasons, and keeps minimal admin actions like drain/shutdown/log-level updates available even when the runtime is in safe mode.
- **Path:** `src/bundle_lifecycle.rs`
  - **Role:** PR-03 bundle lifecycle registry foundation.
  - **Key functionality:** Warms bundles through staged access, discovers provider packs plus runtime wiring metadata, records warm reports including selected provider roles plus hook/subscription wiring snapshots and duplicate-pack conflicts, tracks `Staged`/`Ready`/`Active`/`Draining`/`Retired` state, records lifecycle transition events for warm/activate/rollback/drain-complete actions, and supports activation/rollback plus drain-completion transitions.
  - **Key dependencies / integration points:** Uses `bundle_access`, `domains`, `discovery`, and `runtime_core`; `demo::runner_host` now includes a lifecycle snapshot for the current runtime bundle in status reporting, including the transition-event log, keeps its local bundle source/resolver/fs seams behind a shared active bundle-access handle so lifecycle-driven bundle switches can change live bundle-root reads, swaps the active runtime registry/wiring when activation or rollback changes the selected bundle, syncs the active provider inventory used for canonical provider lookup and resolved-pack execution paths, namespaces host-managed run artifacts by active runtime bundle id under operator state, keys the default local session/state seam storage by active bundle id with fallback reads from the legacy shared paths, promotes missing runtime state forward from the previously active bundle during activation/rollback, resets in-memory session-store indexes on lifecycle transitions so stale wait mappings do not survive bundle switches, records the latest promotion/cleanup/session-index-reset report in the runtime status snapshot, and removes retired bundle runtime-state roots when drain completion is finalized.
...
- **Path:** `.codex/PR.md`
  ...
  - **Key dependencies / integration points:** N/A.
- **Path:** `.codex/PR-00-AUDIT.md`, `.codex/PR-01-runtime-core-and-provider-seams.md` ... `.codex/PR-09-distroless-and-target-runtime-packaging.md`
  - **Role:** Current runtime-core PR planning set.
  - **Key functionality:** `PR-00` now records the concrete audit/removal map for the current repo, and `PR-01` to `PR-09` have been aligned to that audit so later work distinguishes reusable runtime foundations from net-new runtime subsystems. `PR-01` implementation is now using concrete local bundle seams too: `DemoRunnerHost` installs default `BundleSource` / `BundleResolver` / `BundleFs` implementations, provider manifest introspection reads pack bytes through `runtime_core`, post-ingress dispatch target resolution now goes through the same bundle seam instead of rebuilding raw filesystem discovery logic, runtime event/session bundle identity plus `runner_exec` bundle roots are now sourced from `BundleResolver` / `BundleSource` rather than hard-coded `bundle_root` assumptions, and both `DemoRunnerHost` and the standalone `DemoRunner` now pass `PackRuntime::load` provider-backed `DynStateStore` / `DynSessionStore` adapters instead of hidden in-memory defaults.
  - **Key dependencies / integration points:** Guides follow-up operator refactor sequencing.
...
- **Location:** Repo-wide
  - **Evidence:** `ci/local_check.sh` now completes successfully again after fixing the local clippy regression in `src/demo/runner_host.rs`.
  - **Likely cause / nature of issue:** N/A.

## 5. Notes for Future Work
- `src/control_plane.rs` now owns the production control-plane HTTP surface for `/healthz`, `/readyz`, `/status`, `/runtime/drain`, `/runtime/resume`, `/runtime/shutdown`, `/deployments/{stage,warm,activate,rollback,complete-drain}`, `/config/publish`, `/cache/invalidate`, and `/observability/log-level`; mutating requests are leader-gated, auth-hooked, and audited. Deployment stage/warm/activate/rollback/complete-drain now drive real host lifecycle behavior, with `stage` creating an explicit staged lifecycle record and `warm` remaining the capability/wiring analysis step. `config/publish` persists operator-side published config snapshots, `cache/invalidate` clears session indexes plus provider-registry/contract caches, and `observability/log-level` can read or update the live operator log minimum level when the logger is initialized.
